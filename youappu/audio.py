...
def load_mp4(mp4_file_name, file_type='podcast'):
    """
    This tries to load the audio from a named mp3 file.
    It checks the filename has mp3 extension.
    """
    url_pattern = re.compile('^http[s]://')
    s3_pattern = re.compile('^s3://')
    if url_pattern.match(mp4_file_name):
        mp4_file_name = download_file(mp4_file_name, file_type)
    elif s3_pattern.match(mp4_file_name):
        mp4_file_name = download_file_from_s3(mp4_file_name)
    if not mp4_file_name.lower().endswith('.mp4'):
        raise SystemExit(
            'Incorrect audio file format. The file must have .mp3 extension'
        )
    return AudioSegment.from_mp3(mp4_file_name)
